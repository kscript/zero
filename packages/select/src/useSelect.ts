...
export const useSelect = (props: anyObject) => {
  ...
  const showNewOption = computed(() => {
    let hasExistingOption = state.options
      .filter((option: anyObject) => !option.created)
  ...
  const handleQueryChange = (val: any) => {
    ...
    if (props.multiple && props.filterable) {
      nextTick(() => {
        const length =
          (instance.refs.input as HTMLInputElement).value.length * 15 + 20
    ...
    if (props.remote && typeof props.remoteMethod === 'function') {
      ...
      emitter.emit('optionGroup:queryChange')
    } else {
  ...
  const scrollToOption = (option: anyObject) => {
    try {
      ...
      instance.refs.scrollbar &&
        (instance.refs.scrollbar as anyObject).handleScroll()
    } catch (e) {}
  }
  ...
  const getOption = (value: unknown) => {
    ...
    for (let i = cachedOptions.length - 1; i >= 0; i--) {
      ...
      const isEqual = isObject
        ? getValueByPath(cachedOption.props, props.valueKey) ===
  ...
  const managePlaceholder = () => {
    if (state.currentPlaceholder !== '') {
      state.currentPlaceholder = (instance.refs.input as HTMLInputElement).value
        ? ''
  ...
  const resetInputHeight = () => {
    ...
    nextTick(() => {
      ...
      let inputChildNodes = (instance.refs.reference as anyObject).$el
        ?.childNodes
      let input = [].filter.call(
        inputChildNodes,
        (item: HTMLElement) => item.tagName === 'INPUT'
      ...
      if (!Array.isArray(state.selected)) {
        ...
        if (state.selected.length > 0) {
          state.hoverIndex = Math.min.apply(
            null,
            state.selected.map(item => options.indexOf(item))
          )
        } else {
          state.hoverIndex = -1
        }
  ...
  const handleOptionSelect = ([option, byClick = true]: [
    anyObject,
    boolean
  ]) => {
  ...
  const setSoftFocus = () => {
    ...
    ;((instance.refs.input ||
      instance.refs.reference) as HTMLInputElement)?.focus()
  }
  ...
  const toggleMenu = () => {
    if (!selectDisabled.value) {
      ...
      if (state.visible) {
        const input = (instance.refs.input || instance.refs.reference) as
          | HTMLInputElement
          | undefined
        ;((instance.refs.input ||
          instance.refs.reference) as HTMLInputElement)?.focus()
  ...
  const resetInputWidth = () => {
    const width = (instance.refs
      .reference as anyObject).$el?.getBoundingClientRect().width
    if (width !== void 0) {
  ...
  const optionsAllDisabled = computed(() =>
    state.options
      .filter((option: anyObject) => option.visible)
      .every((option: anyObject) => option.disabled)
  )
  ...
  const navigateOptions = (direction: string) => {
    ...
    if (state.options.length === 0 || state.filteredOptionsCount === 0) return
    if (!optionsAllDisabled.value) {
      if (direction === 'next') {
        state.hoverIndex++
        if (state.hoverIndex === state.options.length) {
          state.hoverIndex = 0
        }
      } else if (direction === 'prev') {
        state.hoverIndex--
        if (state.hoverIndex < 0) {
          state.hoverIndex = state.options.length - 1
        }
      }
      const option = state.options[state.hoverIndex] as anyObject
      if (
  ...
  watch(
    ...
    (val: any, oldVal) => {
      if (props.multiple) {
        ...
        if (
          (val && val.length > 0) ||
          (instance.refs.input && state.query !== '')
        ) {
  ...
  watch(
    ...
    () => {
      ...
      let inputs = [].slice.apply(
        instance.vnode.el?.querySelectorAll('input')
      ) as HTMLInputElement[]
  ...
  onMounted(() => {
    ...
    if (reference && reference.el) {
      ...
      state.initialInputHeight =
        ...
        sizeMap[selectSize.value as keyof typeof sizeMap]
    }
  ...
  const optionDestroy = (optionInstance: anyObject) => {
    let selectedOptions = props.multiple ? state.selected as any[] : [state.selected]
    let index = (state.cachedOptions as anyObject[]).indexOf(optionInstance)
    let selectedIndex = selectedOptions.indexOf(optionInstance)
    // if option is not selected, remove it from cache
    if (index > -1 && selectedIndex < 0) {
      state.cachedOptions.splice(index, 1)
    }
    onOptionDestroy((state.options as anyObject[]).indexOf(optionInstance))
  }
  provide(
    'elSelect',
    Object.assign(
